...
export default function Header() {
  ...
  return (
    <header
      className={`fixed left-0 top-0 w-full z-[100] ${poppins.className}`}
    >
      <div className="mw-full mx-auto ">
        <div
          className={`flex items-center justify-between px-8 py-3  transition-all ${isScrolled ? "bg-white/95 backdrop-blur-xl shadow-lg" : "bg-white/90"}`}
        >
          {/* LOGO */}
          <Link href="/">
            <Image src="/logo.svg" alt="Vestigo" width={140} height={30} />
          </Link>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      className={`absolute left-1/2 -translate-x-1/2 -bottom-1 h-[3px] bg-[#C9D600] rounded-full transition-all ${activeMenu === item.name || isSectionActive ? "w-6 opacity-100 shadow-[0_0_10px_rgba(201,214,0,0.8)]" : "w-0 opacity-0"}`}
                    />
                  </Link>
                    ...
                    {item.subLinks && activeMenu === item.name && (
                      ...
                      >
                        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#E3F6FF] via-[#D7EEFF] to-[#F8FDFF] shadow-[0_30px_80px_rgba(28,42,125,0.35)] border border-[#C9D600]/30">
                          ...
                          <div
                            className={`ml-48 p-8 grid gap-4 ${item.subLinks.length > 10 ? "grid-cols-4" : item.subLinks.length > 5 ? "grid-cols-3" : "grid-cols-2"}`}
                          >
                            {item.subLinks.map((sub) => (
                              <Link
                                key={sub.name}
                                href={sub.href}
                                className="group flex flex-row items-center gap-3 p-3 rounded-xl bg-white/40 backdrop-blur-md border border-white/20 hover:bg-white/70 transition-all shadow-sm"
                              >
                                ...
                                <div className="relative">
                                  <p className="font-bold text-[10px] leading-tight uppercase tracking-wider text-[#1C2A7D]">
                                    {sub.name}
                                  </p>
                                </div>
                              </Link>
